...
The portable build copies the Bundle's seven published files. On first launch the desktop launcher stages those files under the Web profile and uses the bundled offline pnpm runtime with the official `dsh plugin` command to register a no-whitespace relative `link:` dependency. A plugin version change refreshes the staged files through the existing link; payload hashes are compared on each launch and `package.json` is copied last so interrupted refreshes are retried. Keeping the link target inside the profile avoids the official Windows shell forwarder's absolute-path whitespace ambiguity. When the portable directory moves, the launcher uses the same official command with a forced offline install to rebuild pnpm's absolute junction from the relative declaration. Normal launches use that profile without an extra patch overlay. Shutdown is checked between asynchronous startup stages, and Electron waits for the active startup task and its owned process tree before exiting. Packaged verification allows both independent 120-second startup limits plus capture time and terminates the complete process tree on timeout. The upstream `dsh` package remains unchanged. Independent users receive the same seven files as an npm tarball from a dedicated `plugin-model-capabilities-v*` GitHub Release whose Draft, checksum, and Latest status are verified separately from portable ZIP Releases.
...
Official application upgrades can replace all upstream packages without removing this capability, provided the portable launcher still ships and registers the Bundle. Existing local profiles are reconciled on the first launch after installation. Public endpoints remain metadata-only unless the user explicitly selects the `always` policy. Saved capability fields remain ordinary user settings if the Bundle is removed.
