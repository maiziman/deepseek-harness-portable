...
默认 ZIP 不含能力插件，启动时也不会安装插件。固定哈希 pnpm 只用于用户明确执行的官方 `dsh plugin` 命令，避免退回 Corepack 或电脑上的全局包管理器。桌面壳会先渲染启动页，再使用内置 Node.js 运行 `dsh web --patch <包内 cedardsh.patch.yml> --no-open --port 0`。这个仅供启动器使用的 patch 会在官方 Web profile 之后加入 CedarDSH 更新与关于界面，不会修改用户的 profile 文件。启动页显示可观测到的目录、运行时、profile 组件、profile、服务和界面阶段以及累计时间；首次创建后备组件链接时，还会把已完成的包内链接数量与构建冒烟测试记录在 `manifest.json` 中的总数比较。桌面壳读取服务输出的本机 URL 后在窗口中打开；再次启动会聚焦已有窗口，关闭窗口会终止整个服务进程树。
...
便携程序只读取严格 `v<语义版本>` 标签、ZIP 版本和附件上传记录相互一致的完整公开 Release。`manifest.json` 必须包含有效的 `portableVersion`。程序每 24 小时最多自动检查一次，把最高的有效便携版本与该字段比较；设置旁的“更新”按钮会立即执行同样的检查。用户接受新版本后，程序会下载精确的 ZIP，校验已公布的大小和 SHA256，验证 manifest 与包所有的顶层项，再只替换这些项。`dsh-home` 和 `workspace` 保留在原位，替换失败时会恢复原有程序文件。如需禁用定时联网检查，可在启动前设置 `DSH_UPDATE_CHECK=0`。
