...
The packaged desktop app reads only complete published Releases whose strict `v<SemVer>` tag, ZIP version, and uploaded asset records agree. `manifest.json` must contain a valid `portableVersion`. At most once every 24 hours the app compares the highest valid portable version with that value; the Update button beside Settings performs the same check immediately. When the user accepts a newer version, the app downloads the exact ZIP, verifies its published size and SHA256, validates its manifest and package-owned entries, and replaces only those entries. `dsh-home` and `workspace` stay in place, and a failed replacement restores the previous program files. Set `DSH_UPDATE_CHECK=0` before launch to disable the scheduled network check.
